        ...
        <div class="mt-4 lg:mt-8 lg:grid lg:grid-cols-4 lg:items-start lg:gap-8">
            ...
            <div class="lg:col-span-3">
                <ul class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <li>
                        <a href="/detail/{{$prod->slug}}" class="group rounded block overflow-hidden border-1 hover:shadow-lg">

                            <img src="/storage/{{$prod->image}}" alt="" class="h-64 w-full object-cover transition duration-400 group-hover:scale-105 sm:h-72" />

                            <div class=" border border-gray-100 bg-white p-6">
                                <div class="input-group hidden">
                                    {{$prod->slug}}
                                </div>
                                <h3 class="mt-0 text-lg font-medium text-gray-900"> {{$prod->name}}
                                </h3>
                                <p class="mt-1.5 text-sm text-gray-700">{{ number_format($prod->price, 0, '.', ',') }} đ</p>
